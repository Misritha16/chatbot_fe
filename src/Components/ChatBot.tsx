...
const ChatBot = () => {
  ...
  const handleChat = async() => {
    ...
    setChatConversation([...chatConversation, { role: "user", msg: value }]);
    setValue("");
    try {
      // return 
      const res: any = await axios.get(`http://127.0.0.3:3000/get?msg=${value}`);
      if (res?.data?.length) {
        setTimeout(() => {
  ...
  console.log('chatConversation --after api call', chatConversation)
